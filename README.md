...
### シリアルデバッグ
- 横のボタン(**Button4**)を押して「Bounce Ball」デモに切り替えたあと、**PlatformIO**の**Serial Monitor**または**Tera Term**等のターミナルアプリを起動すると、加速度の数値が流れてきます。
...
### ソースコード
- **main.cpp**<br>
メインプログラムで、初期化、タイトル表示、サイドボタン(**Button4**)によるデモの切り替え、長押しによるバックライトのオンオフを処理します。
...
- **TinyApp.h .cpp**<br>
複数のアプリを切り替えるときは、このクラスの派生クラスを作成して**main.cpp**に定義するだけで、サイドボタン(**Button4**)で切り替えられます。
...
5つのデモアプリです。**TinyApp**の派生クラスです。
  ...
  **ENV Hat**で温度/湿度を表示します。**Button1**で次のアプリに切り替えます。装着されていないときはスキップします。
  ...
  - **Tetris.h .cpp**<br>
  お馴染みのゲームで、縦向きで遊びます。**Buttton1**長押しで落下します。
